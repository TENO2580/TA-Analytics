...
const RFMEngine = {
  ...
  _calculateScore(values, metric, invert, rules) {
    if (rules && rules.type === 'custom' && rules[metric]) {
      const t = rules[metric];
      return values.map(v => {
        if (metric === 'recency') {
          if (v <= t[0]) return 5;
          if (v <= t[1]) return 4;
   ...
   */
  _segment(r, f, m, rules) {
    if (rules && rules.segments && rules.segments.length > 0) {
      for (const seg of rules.segments) {
        if (r >= seg.rMin && r <= seg.rMax &&
            f >= seg.fMin && f <= seg.fMax &&
            m >= seg.mMin && m <= seg.mMax) {
          return seg.name;
        }
      }
   ...
   */
  calculate(transactions, rules = null) {
    ...
    for (const tx of transactions) {
      ...
      if (!groups[id]) {
        groups[id] = { customer_id: id, customer_name: tx.customer_name, email: tx.email, transactions: [], branches: new Set(), products: new Set() };
      }
      groups[id].transactions.push(tx);
      if (tx.customer_name && !groups[id].customer_name) groups[id].customer_name = tx.customer_name;
      if (tx.email && !groups[id].email) groups[id].email = tx.email;
    ...
    // ── Score ──
    const rScores = this._calculateScore(recencies, 'recency', true, rules);
    const fScores = this._calculateScore(frequencies, 'frequency', false, rules);
    const mScores = this._calculateScore(monetaries, 'monetary', false, rules);

    const wR = rules && rules.weights ? rules.weights.r : 33.33;
    const wF = rules && rules.weights ? rules.weights.f : 33.33;
    const wM = rules && rules.weights ? rules.weights.m : 33.33;

    for (let i = 0; i < customerList.length; i++) {
      const r = rScores[i], f = fScores[i], m = mScores[i];
      customerList[i].r_score = r;
      customerList[i].f_score = f;
      customerList[i].m_score = m;
      customerList[i].rfm_score = `${r}${f}${m}`;
      customerList[i].weighted_score = Math.round((r * wR + f * wF + m * wM) / 10) / 10;
      customerList[i].segment = this._segment(r, f, m, rules);
    }
    ...
    // ── Segment summary ──
    let segmentNames = ['Champions','Loyal Customers','Potential Loyalists','New Customers','Need Attention','At Risk','Lost Customers','Big Spenders','Inactive Customers'];
    if (rules && rules.segments && rules.segments.length > 0) {
      segmentNames = rules.segments.map(s => s.name);
      if (!segmentNames.includes('Other')) segmentNames.push('Other');
    }

    const segments = {};
    for (const s of segmentNames) {
      const custs = customerList.filter(c => c.segment === s);
      if (custs.length === 0 && s === 'Other') continue; // Hide empty Other
      segments[s] = {
        count: custs.length,
  ...
  generateInsights(result) {
    ...
    if (segments['Champions'] && segments['Champions'].count > 0) {
    ...
    let churnCount = 0;
    let churnRev = 0;
    ['At Risk', 'Lost Customers', 'Inactive Customers'].forEach(s => {
      if (segments[s]) { churnCount += segments[s].count; churnRev += segments[s].revenue; }
    });
    ...
    // ── At Risk deep dive ──
    if (segments['At Risk'] && segments['At Risk'].count > 0) {
    ...
    // ── Lost customers ──
    if (segments['Lost Customers'] && segments['Lost Customers'].count > 0) {
    ...
    if (segments['New Customers'] && segments['New Customers'].count > 0) {
    ...
    // ── Big spenders ──
    if (segments['Big Spenders'] && segments['Big Spenders'].count > 0) {
    ...
    // ── Potential loyalists ──
    if (segments['Potential Loyalists'] && segments['Potential Loyalists'].count > 0) {
      ins.push({ icon: '⭐', cat: 'Growth', priority: 'medium',
    ...
    if (churnCount > 0) {
      let recPot = 0;
      if (segments['At Risk']) recPot += segments['At Risk'].revenue * 0.3;
      if (segments['Lost Customers']) recPot += segments['Lost Customers'].revenue * 0.1;
      
      const recoveryPotential = Math.round(recPot);
