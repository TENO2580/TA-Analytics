...
const DataCleaner = {
  ...
  detectColumns(rows) {
    ...
    const map = {
      ...
      email:       this._findCol(headers, ['email','mail','customer_email']),
    };
    const missing = [];
  ...
  /**
   * Validate row count based on subscription plan
   */
  validateRowCount(rows) {
    ...
    if (typeof Subscription !== 'undefined') {
      ...
      // Fallback
      plan = localStorage.getItem('teno_plan') || 'free';
      if (plan === 'growth') limit = 10000;
      else if (plan === 'pro') limit = Infinity;
    }
    
    if (rows.length > limit) {
      const msg = plan === 'growth' ?
        `Growth plan supports up to 10000 rows. Upgrade to Pro Unlimited for larger datasets.` :
        `Free plan supports up to 1000 rows. Upgrade for larger datasets.`;
